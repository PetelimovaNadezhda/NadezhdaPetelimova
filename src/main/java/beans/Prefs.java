...
public class Prefs {
    ...
    public String toString() {
        return new ToStringBuilder(this).append("permissionLevel", permissionLevel).append("voting", voting).append("comments", comments).append("invitations", invitations).append("selfJoin", selfJoin).append("cardCovers", cardCovers).append("cardAging", cardAging).append("calendarFeedEnabled", calendarFeedEnabled).append("background", background).append("backgroundImage", backgroundImage).append("backgroundImageScaled", backgroundImageScaled).append("backgroundTile", backgroundTile).append("backgroundBrightness", backgroundBrightness).append("backgroundColor", backgroundColor).append("backgroundBottomColor", backgroundBottomColor).append("backgroundTopColor", backgroundTopColor).append("canBePublic", canBePublic).append("canBeEnterprise", canBeEnterprise).append("canBeOrg", canBeOrg).append("canBePrivate", canBePrivate).append("canInvite", canInvite).toString();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder().append(backgroundBrightness).append(backgroundColor).append(comments).append(backgroundTopColor).append(canBeEnterprise).append(backgroundImage).append(canBeOrg).append(backgroundBottomColor).append(voting).append(calendarFeedEnabled).append(backgroundTile).append(canBePublic).append(canBePrivate).append(backgroundImageScaled).append(permissionLevel).append(cardAging).append(canInvite).append(invitations).append(background).append(cardCovers).append(selfJoin).toHashCode();
    }
